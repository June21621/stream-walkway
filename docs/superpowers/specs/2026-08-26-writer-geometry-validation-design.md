...
#### 재검토 (2026-08-27) — 거부 유지

실측으로 성격이 바뀌었으므로 "프로덕션 동작을 바꾸면서까지 막을 값어치가 있나"를 다시 물었다.
아래 사실을 확인한 뒤 **거부를 유지**하기로 했다.

먼저, 원래 있었을 법한 논거 하나는 성립하지 않는다. **빈 지오메트리는 읽기 경로를 깨지 않는다.**
JTS 1.19.0에서 `POINT EMPTY`의 `toText()`는 `"POINT EMPTY"`를 반환하고 `getCoordinates()`는
길이 0 배열을 반환한다. `TrailView.from()`/`StreamView.from()`의
`toText().replaceFirst("\s+\(", "(")`는 괄호가 없어 매치되지 않으므로 그대로 통과하고,
API는 `"location": "POINT EMPTY"` 문자열을 낸다 — 예외도 NPE도 없다. 따라서 "막지 않으면
하류가 터진다"는 근거는 쓸 수 없다.

유지하는 실제 근거는 세 가지다.

1. **스키마가 이미 위치를 필수로 선언한다.** `streams.location`과 `trails.location`은 둘 다
   `NOT NULL`이다. 그런데 `POINT EMPTY`는 좌표가 없으면서 `NOT NULL`을 통과한다 — 의미상
   NULL과 같은 값이 제약을 우회하는 것이다. 이 관점에서 EMPTY 거부는 새 정책이라기보다
   `NOT NULL`이 원래 의도한 바를 강제하는 쪽에 가깝다.
2. **지금이 막기 가장 싼 시점이다.** `location`은 현재 저장·표시 전용이다 — main 소스 전체에
   `ST_` 공간 쿼리가 없고 `location`에 GIST 인덱스도 없다. frontend도 아직 이 값을 소비하지
   않는다. 나중에 지도 렌더링을 붙일 때 `"POINT EMPTY"` 행이 쌓여 있으면 그때는 데이터 정리가
   함께 필요해진다.
3. **두 엔드포인트가 대칭이어야 한다는 원래 판단은 그대로다.** 오히려 실측 결과 Trail·Stream이
   같은 성격(정책)이 되어 더 일관된다.

기각한 대안: EMPTY 검사 제거(= Z/M/ZM 버그 수정만 남김)는 `NOT NULL` 우회 구멍을 그대로
남긴다. 별도 PR로 분리하는 안은 검사 자리와 테스트가 이미 한 덩어리라 되돌리고 재구성하는
비용에 비해 얻는 게 적다.

**후속 항목:** "위치를 아직 모르는 카메라"라는 상태가 도메인상 필요하다면, `POINT EMPTY`라는
우회 대신 `location`을 NULL 허용으로 바꿔 정식으로 표현하는 것이 맞다. 이번 범위 밖이다.
