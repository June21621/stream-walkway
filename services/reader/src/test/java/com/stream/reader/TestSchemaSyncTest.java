...
//
// 이 테스트가 왜 필요한가: 리뷰에서 reader의 schema.sql에 있는 제약 6개(FK 3개,
// UNIQUE 1개, CHECK 2개)를 전부 지워도 reader 전체 테스트 스위트가 그대로
...
//
// ⚠️ 이 클래스는 writer 모듈에도 똑같이 존재한다. 고의적인 중복이다 — 지우지
...
class TestSchemaSyncTest {
    ...
    // TrailCommandHandlerPostgresTest(writer 모듈)가 init-db.sql을 찾을 때와 같은
    // 방식으로, 모듈 베이스 디렉터리(예: services/reader) 기준 상대 경로를 쓴다.
    private static final Path OWN_SCHEMA = Path.of("src", "test", "resources", "schema.sql");
    private static final Path WRITER_SCHEMA = Path.of("..", "writer", "src", "test", "resources", "schema.sql");
    private static final Path OWN_DATA = Path.of("src", "test", "resources", "data.sql");
    private static final Path WRITER_DATA = Path.of("..", "writer", "src", "test", "resources", "data.sql");

    // schema.sql 쌍과 data.sql 쌍을 한 테스트에서 함께 검증한다. SoftAssertions를
    // 써서 둘 중 하나만 깨져도 조용히 넘어가지 않고, 두 쌍이 동시에 어긋나면
    // 실패 메시지에 둘 다 나열한다.
    @Test
    @DisplayName("schema.sql과 data.sql - reader와 writer 사본이 각각 바이트 단위로 동일하다")
    void schemaAndDataSqlMatchWriterCopies() throws IOException {
        SoftAssertions softly = new SoftAssertions();
        assertIdenticalBytes(softly, OWN_SCHEMA, WRITER_SCHEMA);
        assertIdenticalBytes(softly, OWN_DATA, WRITER_DATA);
        softly.assertAll();
    }
